...
class RemoteConfigManager:
    ...
    async def fetch_config(self, source_name: str, force_refresh: bool = False) -> Optional[Dict[str, Any]]:
        ...
        if not force_refresh and source_name in self.configs:
            ...
            if datetime.now() < cached.expires_at:
                logger.debug(f"Using cached config for {source_name}")
                return cached.data
        ...
        try:
            ...
            # Fetch from remote
            logger.info(f"Fetching config from {source_name}: {source.url}")
            ...
            logger.info(f"Successfully fetched config from {source_name}")
            return config_data
            
        except Exception as e:
            logger.error(f"Failed to fetch config from {source_name}: {e}")
    ...
    async def _save_to_cache(self, source_name: str, data: Dict[str, Any]):
        ...
        try:
            ...
            logger.debug(f"Saved config cache for {source_name}")
        except Exception as e:
        ...
        if source.fallback_file and Path(source.fallback_file).exists():
            try:
                async with aiofiles.open(source.fallback_file, 'r') as f:
                    content = await f.read()
                    data = json.loads(content)
                    logger.info(f"Using fallback file for {source_name}")
                    return data
            except Exception as e:
                logger.error(f"Failed to read fallback file: {e}")
        
        logger.error(f"No fallback available for {source_name}")
        return None
    ...
    async def refresh_all_configs(self):
        ...
        if tasks:
            ...
            logger.info(f"Refreshed {success_count}/{len(tasks)} config sources")
